...
class NeuralNetwork:
    ...
    def plot_test(self, input_set, output_set) :
        plt.clf() #we clean the previous plot
        plt.figure(figsize=(8, 8))
        for i, sample in enumerate(input_set):
            self.forward_pass(sample)
            prediction = [round(neuron.activated_value, 2) for neuron in self.total_layers[-1]]
            print(f"Predicted: {prediction} | Reference: {output_set[i]}")
            plt.scatter(sample[0], sample[1], color='blue' if prediction[0] > 0.5 else 'red')
        plt.xlabel('X')
        plt.ylabel('Y')
        plt.grid(True)
        plt.savefig("PREDICTION_points.png")
        print("Plot saved as 'PREDICTION_points.png'")
